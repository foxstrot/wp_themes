...
if (!function_exists('add_scripts')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_scripts() { // добавление скриптов
	    ...
	    wp_enqueue_script('jquery','//ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js','',NETVOXLAB_OWNRADIO_THEME_VERSION,true); // добавляем свой
	    wp_enqueue_script('bootstrap', get_template_directory_uri().'/js/bootstrap.min.js','',NETVOXLAB_OWNRADIO_THEME_VERSION,true); // бутстрап
	    wp_enqueue_script('main', get_template_directory_uri().'/js/main.js','', NETVOXLAB_OWNRADIO_THEME_VERSION , true); // и скрипты шаблона
	}
}
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
	    if(is_admin()) return false; // если мы в админке - ничего не делаем
		wp_enqueue_style( 'ownradio', get_template_directory_uri().'/font-awesome/css/font-awesome.min.css', '', NETVOXLAB_OWNRADIO_THEME_VERSION ); // font-awesome
	    wp_enqueue_style( 'bs', get_template_directory_uri().'/css/bootstrap.min.css', '', NETVOXLAB_OWNRADIO_THEME_VERSION ); // бутстрап
		wp_enqueue_style( 'main', get_template_directory_uri().'/style.css', '', NETVOXLAB_OWNRADIO_THEME_VERSION  ); // основные стили шаблона
	}
}
